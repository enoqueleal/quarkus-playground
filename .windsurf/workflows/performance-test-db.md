---
description: Complete performance testing workflow for People API with database integration using tool calls
---

# Performance Testing Workflow - People API with Database Integration

This workflow automates the complete performance testing process for People API with PostgreSQL database integration using available tool calls.
...
Run this workflow to perform complete performance testing of the People API with database integration. The workflow will:
...
The test targets the upstream-camel service at `http://localhost:8082/api/people` which calls the downstream People API with database integration, measuring end-to-end performance including database operations.

## Notes

- Container limits: 1 vCPU, 1GB RAM
- Test endpoint: http://localhost:8082/api/people (upstream-camel service)
- Single test file: `.k6/scripts/k6-unified-test-with-db.js`
- Progressive load: 10→50→100→200→400→600→800→1000 VUs
- Reports saved in: `.k6/reports/` directory
...
- Target service: Upstream-camel service calling downstream People API with database operations